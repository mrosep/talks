...
\setbeamertemplate{footline}{%
  ...
  \hbox to \paperwidth{%
    ...
    \begingroup
    \setlength{\fboxsep}{2pt}% Padding inside the box
    \colorbox{firebrick}{\makebox[\dimexpr0.333\paperwidth\relax][c]{\strut\textcolor{white}{2411.17663}}}%
    \endgroup
...
%%%
\title[Accelerated nested sampling with applications to cosmology and gravitational waves] %optional
{Accelerated nested sampling with applications to cosmology and gravitational waves}

\author[Metha Prathaban] % (optional)
{Metha Prathaban \break myp23@cam.ac.uk} %\break %\vfill\includegraphics[width=0.16\textwidth]{Ca_Foscari Beamer/qr-code.png}}
\date{}
...
\begin{frame}{Inverse problems in GW physics}\vfill
    ...
    \begin{tikzpicture}[node distance=5cm, auto]
    % Nodes
    \node (params) [draw, rectangle, rounded corners, minimum width=3cm, minimum height=1cm, align=center] {model parameters \\ \textcolor{firebrick}{$\boldsymbol{\theta = \mathcal{M}}\boldsymbol{,q, d_L, t_c ...}$}};
    ...
    \node (params2) [draw, rectangle, rounded corners, minimum width=3cm, minimum height=1cm, align=center, below of=params, yshift=2cm] {model parameters, inferred \\ \textcolor{firebrick}{$\boldsymbol{\theta}_{\text{est}}$}};
...
\vfill
    ...
    \visible<6>{\draw[explain, <-, thick, firebrick](a4)--++(1.5,1)node[above]{resolution};}
    \visible<4-7>{\draw[explain, <-, thick, firebrick](a1)--++(-2,1)node[above]{likelihood evaluation time};}
    \visible<5-8>{\draw[explain, <-, thick, firebrick](a2)--++(-2.8,-1.7)node[below]{drawing new live point};}
    ...
    \visible<5>{}
    \visible<3-9>{\draw[explain, <-, thick, firebrick](a3)--++(1.2,-0.6)node[below]{compression from \textcolor{BurntOrange}{prior} to \textcolor{RoyalBlue}{posterior} ($\approx \mathrm{ln}\frac{V_\pi}{V_\mathcal{P}}$)};}
    
    \visible<7->{\draw[explain, <-, thick](a4)--++(1.5,1)node[above]{resolution (baked in)};}
    \visible<8->{\draw[explain, <-, thick](a1)--++(-2,1)node[above]{faster waveform models, \textsc{CosmoPower}};}
     \visible<9->{\draw[explain, <-, thick](a2)--++(-2.8,-1.7)node[below]{better samplers};}
     \visible<2>{\draw[explain, <-, thick, firebrick](a3)--++(1.2, -0.6)node[below]{focus of this talk};}
    \end{tikzpicture}
\end{frame}
...
%         \onslide<4->{\tikz\node[draw=firebrick, inner sep=8.5pt, circle, overlay, shift={(-8pt,2pt)}]{}}  
%         \tikzmarknode{DKL}{\onslide<3->{\hspace*{-1.6em} \mathcal{D}_{\mathrm{KL}}}} 
%         \onslide<3->{\times} 
%         \tikzmarknode{nlive}{\onslide<3->{n_\textrm{live}}} 
%     \end{equation}
%     \begin{tikzpicture}[remember picture, overlay]
%         \onslide<1->{\draw[explain, <-, thick, firebrick] (Tl)--++(-2,1) 
%         node[above]{likelihood evaluation time};}

%         \onslide<1>{\draw[explain, <-, thick, firebrick] (nl)--++(2,-1) 
%         node[below]{number of likelihood evaluations};}

%         \onslide<2->{\draw[explain, <-, thick, firebrick] (fsampler)--++(-3,-1.7) 
...
%         \onslide<2>{\draw[explain, <-, thick, firebrick] (niter)--++(1,1) 
%         node[above]{number of iterations};}

%         \onslide<3>{\draw[explain, <-, thick, firebrick] (DKL)--++(1.2,-0.8) 
...
%         \onslide<3->{\draw[explain, <-, thick, firebrick] (nlive)--++(1,1) 
%         node[above]{resolution};}

%         \onslide<4>{\draw[explain, <-, ultra thick, firebrick] (DKL)--++(1.2,-0.8) 
%         node[below]{focus of this talk!};}
%     \end{tikzpicture}
% \end{frame}
...
%     \end{equation}
%     \begin{tikzpicture}[remember picture, overlay]
%         \onslide<1-2>{\draw[explain, <-, thick, firebrick] (Tl)--++(-2,1) 
%         node[above]{likelihood evaluation time};}
%         \onslide<3->{\draw[explain, <-, thick, black] (Tl)--++(-2,1) 
%         node[above]{faster waveform models};}

%         \onslide<1-3>{\draw[explain, <-, thick, firebrick] (fsampler)--++(-3,-1.7) 
%         node[below]{drawing new live point};}
...
%         \onslide<1->{\draw[explain, <-, thick, firebrick] (DKL)--++(1.2,-0.8) 
%         node[below]{compression between \textcolor{BurntOrange}{prior} and \textcolor{RoyalBlue}{posterior} ($\approx \mathrm{log}\frac{V_\pi}{V_\mathcal{P}}$)};}

%         \onslide<1>{\draw[explain, <-, thick, firebrick] (nlive)--++(1,1) 
%         node[above]{resolution};}
