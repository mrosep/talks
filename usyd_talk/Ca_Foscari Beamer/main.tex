...
{Scaling nested sampling for next generation gravitational-wave data \\ \vspace{0.5em}
}
...
\begin{itemize}
    ...
    \item My interests are all things Bayesian inference in the context of GWs!
\end{itemize} 
\vspace{1em}
\bblock{\begin{center}
The Handley group!
...
\begin{itemize} 
\item Short-duration signals from compact binary coalescences
...
\end{itemize} 
\eblock
\end{minipage}}\hfill                                       \visible<2->{ \begin{minipage}{0.47\textwidth}\vspace{0.6cm}                        \centering                                          
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item New GW sources (not just CBCs)
        \item \textcolor{firebrick}{\textbf{Still doable with current pipelines?}}
    \end{itemize}
    \eblock
\end{minipage}}                                                    
\end{frame}
...
\begin{frame}{Scaling of stochastic sampling}
    \begin{equation}
...
\begin{frame}{Machine learning accelerated NS}
    \begin{equation}
...
%     \item<3-> Use this as 'repartitioned prior' for high resolution run (and correct evidences and posteriors accordingly).
...
\vfill
  \begin{itemize}
    \item \textbf{CPUs (Central Processing Units):}
      \begin{itemize}
        ...
        \item Optimised for wide range of tasks and excel at sequential, complex calculations and intricate decision-making.
  ...
  \end{itemize}
\vfill
\bblock{ \centering Advantages of GPUs for GW inference}
\begin{itemize}
\item \textbf{Likelihood Evaluation:} 
 Frequency bins are independent, so waveforms can be evaluated across many bins in parallel.
      ...
      \end{itemize}
\eblock
% \bblock{\centering\begin{itemize}
%         \item \textbf{Likelihood Evaluation:} 
%         \begin{itemize}
...
\begin{frame}{Conclusions}
    ...
    \begin{itemize}
        \item 3G era presents significant challenges for data analysis
        \vspace{1em}
        \begin{itemize}
            \item Current standard stochastic sampling methods, including traditional nested sampling, will not scale well
        \end{itemize}
        \vspace{1em}
       ...
       \vspace{1em}
        \begin{itemize}
                \item GPUs are becoming the norm in many areas, including astrophysics, and computing power will become heavily skewed towards GPUs.
                \item Adapting our algorithms to modern hardware is essential to keep pace with the cutting edge of computing power.
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{\textsc{blackjax ns}}
    ...
    \bblock{\centering \textsc{blackjax NS}}
        \begin{itemize}
            ...
            \item Exploits parallel capabilites of GPUs to accelerate sampling procedure
            \item Combined with \textsc{ripple}, can do GW inference on GPU
            \item Fast - 11 parameter nested sampling run on GW150914 in minutes
            \item Can run on CPUs too - many new features incorporated which enhance core nested sampling algorithm, and especially useful in context of GWs.
        \end{itemize}
    \eblock
    }
\end{frame}
